function [ContStruc,K,best_rho,feas]=Optimize_ContStruc_DT_H2(F,Gdec,Hdec,N,Gw)
...
%LMI_CT_H2 which has to be changed based on the requested control
...
[K,rho,feas]=LMI_DT_H2(F,Gdec,Hdec,N,ContStruc,Gw);
...
while (counter<=100) %change this threshold to reduce/increment iterations after rho stops decreasing
    ...
    if ContStruc_new == ContStruc
        ...
        [new_K,new_rho,new_feas]=LMI_DT_H2(F,Gdec,Hdec,N,ContStruc_new,Gw);
