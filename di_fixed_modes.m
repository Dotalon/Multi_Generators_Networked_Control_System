...
Ctot=[];
for i=1:N
    m(i)=size(Baggr{i},2);  % #coloumns of Baggr{1} (which is Bdec{1} in the main, =1)
    p(i)=size(Caggr{i},1); % #rows of Caggr{1}, which is Bdec{1} in the main, =3
    Btot=[Btot,Baggr{i}];
    Ctot=[Ctot
        Caggr{i}];
end

m_tot=size(Btot,2);  % #cols of Btot, =10
p_tot=size(Ctot,1); %=1
